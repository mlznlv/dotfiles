...
dotfiles_render_selection() (
    ...
    trap 'rm -rf -- "$DOTFILES_RENDER_PRIVATE"' EXIT
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    ...
    mkdir "$local_staging" "${local_private}/destination" "${local_private}/cache" || return 4
    ...
    for local_index in "${!local_targets[@]}"; do
        dotfiles_render_prepare_target "$local_output_resolved" "${local_targets[$local_index]}" || {
            printf 'error: internal render output target is unsafe\n' >&2
            return 3
        }
    done
    for local_index in "${!local_targets[@]}"; do
        cp -- "${local_staging}/${local_targets[$local_index]}" "${local_output_resolved}/${local_targets[$local_index]}" || return 4
    done
)
