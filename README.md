...
- [Starship](https://starship.rs/) — shell prompt rendering and preset support.
...
## Prompt: Starship presets

Starship owns prompt rendering. Zsh completion, history, fzf, zoxide, autosuggestions, and syntax highlighting remain independent of the prompt.

The default preset is Starship's official:

```text
plain-text-symbols
```

This is the preset documented at `https://starship.rs/presets/plain-text` and generated by Starship itself with:

```bash
starship preset plain-text-symbols
```

The repository does not copy preset TOML into Git. Instead, the selected preset name is machine-local and Starship generates its config into a versioned cache. This keeps official presets upgradeable and avoids `chezmoi diff` when switching UI styles.
...
Switch to any preset supported by the installed Starship version:
...
`plain-text` is accepted as a convenience alias:

```bash
prompt-preset plain-text
exec zsh -l
```
...
Persistent machine-local selection is stored at:

```text
~/.config/starship/preset
```

Generated preset configs are cached by Starship version under:

```text
~/.cache/starship/presets/<starship-version>/<preset>.toml
```

Neither path belongs in Git.

Starship itself is part of the macOS base Brewfile. On a machine where Starship has not been provisioned yet, the shell falls back to a minimal native Zsh prompt instead of breaking startup.
...
If a selected preset is no longer available in the installed Starship version, startup falls back to `plain-text-symbols`.
...
- `homebrew/*` — real Homebrew declarations for macOS, including Starship.
...
`prompt.zsh` initializes Starship and manages only machine-local preset selection/caching. It does not contain a custom handcrafted prompt theme.
...
- `~/.config/starship/preset` or generated Starship preset cache;
