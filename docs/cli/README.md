...
## Proposed local selection workflow
...
proposes later `config set`, `config interactive`, `config inspect`, and
...
The proposed save commands change only the CLI-owned selection file and never
...
flag-based selection only after ADR 0011 is accepted.
