...
Only `apply` mutates managed home targets; `config set` changes only its
CLI-owned local selection file. No command installs software.
...
> available. Flag-based local selection is available, but selection-consuming
> commands do not load it yet.
...
4. Optionally save the same explicit intent with `config set`.
...
The command changes only the standard local selection file and necessary owned
configuration directories. It never checks prerequisites, renders, plans,
applies, or installs software. See the [config set reference](config/set.md)
for exact paths, output, recovery, and concurrency limits.

Current selection-consuming commands still require an explicit `--profile` or
`--modules` base and do not read or combine with saved state. Interactive
selection, consumption, inspect, and doctor remain later focused increments in
the [Phase 4 roadmap](../roadmap.md#phase-4-configuration-workflow).
...
path and permissions, before rerunning `config set`. The command never repairs,
normalizes, or deletes existing state. See [config set](config/set.md) for lock
...
only when `config set` safely compares or updates it. It does not open or invoke
prerequisites, use the network, request elevated privileges, call software
providers, or display destination or state contents. Apply delegates only
confirmed, freshly verified selected files to Chezmoi and performs no rollback
or removal.
